...
const TaskItem: React.FC<TaskItemProps> = React.memo(({
  ...
  // Safely access tags with fallback
  const tags = task.tags || [];
  const timeRequirement = tags.find(tag => tag.includes('min'))?.match(/(\d+)\s*min/)?.[1];
...
const RoutineExpandedView: React.FC<RoutineExpandedViewProps> = ({
  ...
  // Sort tasks by dependencies - FIXED LOGIC
  const sortedTasks = useMemo(() => {
    ...
    const visit = (taskId: string) => {
      ...
      // Find tasks that depend on this task (not the other way around)
      const dependencies = routine.taskDependencies || [];
      const tasksThatDependOnThis = dependencies
        .filter(dep => dep.taskId === taskId)
        .map(dep => tasks.find(t => dependencies.some(d => 
          d.order > dep.order && d.taskId === t.id
        )))
        .filter(Boolean);
      
      // Visit dependencies first (tasks that need to be done before this one)
      const dependenciesForThisTask = dependencies.find(d => d.taskId === taskId);
      if (dependenciesForThisTask && dependenciesForThisTask.order > 0) {
        // Find tasks with lower order numbers
        const prerequisiteTasks = dependencies
          .filter(d => d.order < dependenciesForThisTask.order)
          .map(d => taskMap.get(d.taskId))
          .filter(Boolean) as Task[];
        
        prerequisiteTasks.forEach(preTask => {
          if (!visited.has(preTask.id)) {
            visit(preTask.id);
          }
        });
      }
  ...
  // Check if task is locked (simplified logic)
  ...
  // Calculate average completion time (safe access to tags)
  const averageTime = useMemo(() => {
    const timeReqs = tasks
      .map(t => {
        const tags = t.tags || [];
        return tags.find(tag => tag.includes('min'))?.match(/(\d+)\s*min/)?.[1];
      })
      .filter(Boolean)
      .map(Number);
...
// Enhanced component with database observables - FIXED
const enhance = withObservables(['routine'], ({ routine }: { routine: Routine }) => ({
  routine: routine.observe(),
  tasks: routine.tasks ? routine.tasks.observe() : [],
}));

...
interface TaskExpandedViewDirectProps {
  task: Task;
  visible: boolean;
  onClose: () => void;
...
export const TaskExpandedViewDirect: React.FC<TaskExpandedViewDirectProps> = ({
  task,
  visible,
  onClose,
  onEdit,
  ...
  // Load linked goal and routine
  useEffect(() => {
    const loadLinkedItems = async () => {
      if (task.goalId) {
        try {
          const linkedGoal = await goalRepository.findById(task.goalId);
          setGoal(linkedGoal);
        } catch (error) {
      ...
      if (task.routineId) {
        try {
          const linkedRoutine = await routineRepository.findById(task.routineId);
          setRoutine(linkedRoutine);
        } catch (error) {
          console.error('Error loading linked routine:', error);
        }
      }
    };
  ...
  }, [task.goalId, task.routineId, visible]);

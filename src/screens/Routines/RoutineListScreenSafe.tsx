import React, { useEffect, useState, useCallback, useMemo, Component, ErrorInfo, Suspense } from 'react';
import {
  View,
  Text,
  FlatList,
...
const RoutineCard = React.lazy(() => import('../../components/routines/RoutineCard'));
const RoutineExpandedView = React.lazy(() => import('../../components/routines/RoutineExpandedView'));
...
const RoutineListScreenComponent: React.FC<RoutineListScreenProps> = ({ navigation }) => {
  ...
  // Using state for routines
  const [routines, setRoutines] = useState<Routine[]>([]);

  // Check database on mount
  useEffect(() => {
    console.log('[RoutineListScreen] Checking database...');
    const checkDatabase = async () => {
      try {
        ...
        if (db) {
          ...
          console.error('[RoutineListScreen] Database not initialized');
          setError('Database not initialized');
        }
      } catch (err) {
        console.error('[RoutineListScreen] Error checking database:', err);
          ...
          { text: 'Cancel', style: 'cancel' },
          {
            ...
            onPress: async () => {
              try {
                ...
                await loadRoutines(); // Reload routines
              } catch (error) {
  ...
  const handleCreateRoutine = useCallback(async () => {
    ...
    try {
      ...
      await routineRepository.createRoutine({
        name: `Test Routine ${routineCount}`,
        description: 'A test routine to verify functionality',
        icon: ['☀️', '🌙', '⭐', '🏃', '📚'][routineCount % 5],
        color: ['ocean', 'forest', 'sunset', 'lavender', 'coral'][routineCount % 5],
        scheduleType: 'daily',
  ...
  }, [loading, databaseReady, error]);

  console.log('[RoutineListScreen] Main render - routines:', routines.length, 'loading:', loading, 'error:', error, 'dbReady:', databaseReady);
...
export const RoutineListScreen: React.FC<RoutineListScreenProps> = (props) => {
